 ...
 */
data class ScaleCanvasViewPort(
    val scale: Float,
    val visualRect: Rect,
)

/**
 * 从ScaleImageView状态直接获取当前视口对象
 *
 * @return
 */
fun ScaleState.getViewPort(): ScaleCanvasViewPort {
 ...
 *
 * @param scaleDecoder 图片加载器
 * @param viewPort 视口
 */
@Composable
fun ScaleCanvas(
    scaleDecoder: ScaleDecoder,
    viewPort: ScaleCanvasViewPort,
) {
    ...
    BoxWithConstraints(
        ...
        val needRenderHeightTexture by remember(maxWidthPx, maxHeightPx) {
            derivedStateOf {
                ...
                BigDecimal(scaleDecoder.decoderWidth)
                    .multiply(BigDecimal(scaleDecoder.decoderHeight)) > BigDecimal(maxHeightPx.toDouble())
        ...
        val inSampleSize by remember(realWidth) {
            derivedStateOf {
                calculateInSampleSize(
                    srcWidth = scaleDecoder.decoderWidth,
        ...
        var bitmap by remember { mutableStateOf(scaleDecoder.thumbnail) }
        LaunchedEffect(backgroundInputSample) {
            scope.launch(Dispatchers.IO) {
                bitmap = scaleDecoder.decodeRegion(
                    backgroundInputSample, android.graphics.Rect(
                        0,
                        0,
                        scaleDecoder.decoderWidth,
                        scaleDecoder.decoderHeight
                    )
                )
            }
        }
        ...
        LaunchedEffect(key1 = renderHeightTexture) {
            if (!renderHeightTexture) {
                scaleDecoder.renderQueue.clear()
                scaleDecoder.clearAllBitmap()
            }
        }
        ...
        fun updateRenderList() {
            ...
            // 计算当前渲染方块大小
            val renderBlockSize =
                scaleDecoder.blockSize * (realWidth.div(scaleDecoder.decoderWidth))
            var tlx: Int
            var tly: Int
            var startX: Float
            var startY: Float
            ...
            for ((column, list) in scaleDecoder.renderList.withIndex()) {
                ...
                for ((row, block) in list.withIndex()) {
                    ...
                    if (block.inBound) {
                        if (!scaleDecoder.renderQueue.contains(block)) {
            ...
            scope.launch(Dispatchers.IO) {
                synchronized(scaleDecoder.renderQueue) {
                    insertList.forEach {
                        scaleDecoder.renderQueue.putFirst(it)
                    }
                    removeList.forEach {
                        scaleDecoder.renderQueue.remove(it)
                    }
                }
            }
        }
        ...
        LaunchedEffect(realWidth, realHeight, viewPort.visualRect) {
            ...
            scope.launch(Dispatchers.IO) {
                ...
                scaleDecoder.setMaxBlockCount(blockDividerCount)
        ...
        Canvas(
            ...
            withTransform({
                ...
                if (renderHeightTexture && !calcMaxCountPending) {
                    scaleDecoder.forEachBlock { block, _, _ ->
                        block.getBitmap()?.let {
                            drawImage(
