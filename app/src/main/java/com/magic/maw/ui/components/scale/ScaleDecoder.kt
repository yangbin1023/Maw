...
/**
 * 用以提供SamplingCanvas显示大型图片，rememberScaleDecoder，createScaleDecoder
 *
 ...
 *
 * @param decoder
 * @param rotation 请参考ScaleDecoder.Rotation
 * @return
 */
fun createScaleDecoder(
    decoder: BitmapRegionDecoder,
    rotation: ScaleDecoder.Rotation = ScaleDecoder.Rotation.ROTATION_0,
): ScaleDecoder {
    return ScaleDecoder(decoder = decoder, rotation = rotation).apply {
 ...
 *
 * @param file
 * @return ScaleDecoder成功创建时不为空，创建过程中出现异常会返回Exception
 */
@Composable
fun rememberScaleDecoder(file: File): Pair<ScaleDecoder?, Exception?> {
    val samplingDecoder = remember { mutableStateOf<ScaleDecoder?>(null) }
    val expectation = remember { mutableStateOf<Exception?>(null) }
    LaunchedEffect(file) {
        launch(Dispatchers.IO) {
            try {
                samplingDecoder.value = createScaleDecoder(file)
            } catch (e: Exception) {
                expectation.value = e
            }
        }
 ...
 */
@Composable
fun rememberScaleDecoder(
    inputStream: InputStream,
    rotation: ScaleDecoder.Rotation = ScaleDecoder.Rotation.ROTATION_0,
): Pair<ScaleDecoder?, Exception?> {
    val samplingDecoder = remember { mutableStateOf<ScaleDecoder?>(null) }
    val expectation = remember { mutableStateOf<Exception?>(null) }
    LaunchedEffect(inputStream) {
        launch(Dispatchers.IO) {
            try {
                ...
                samplingDecoder.value = createScaleDecoder(decoder, rotation)
            } catch (e: Exception) {
                expectation.value = e
            }
        }
